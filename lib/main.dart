...
Future<void> main(List<String> arguments) async {
  ...
  switch (os) {
    case 'ANDROID':
      command =
      '\$lflutter/flutter build apk --release --dart-define-from-file=$envFile --build-number=$buildNumber --build-name=$buildName --flavor ${type.toLowerCase()} -t $entrypoint';
      break;
    case 'APPBUNDLE':
      command =
      '\$lflutter/flutter build appbundle --release --dart-define-from-file=$envFile --build-number=$buildNumber --build-name=$buildName --flavor ${type.toLowerCase()} -t $entrypoint';
      break;
    case 'IOS':
      command =
      '\$lflutter/flutter build ipa --dart-define-from-file=$envFile --build-number=$buildNumber --build-name=$buildName --flavor ${type.toLowerCase()} -t $entrypoint';
      break;
    case 'WEB':
      command =
      '\$lflutter/flutter build web --web-renderer html --dart-define-from-file=$envFile -t $entrypoint';
      break;
    default:
