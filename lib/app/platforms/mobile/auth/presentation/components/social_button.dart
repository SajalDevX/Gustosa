...
import 'package:gustosa/app/platforms/mobile/auth/presentation/bloc/sign_up_bloc/bloc.dart';
import 'package:gustosa/app/platforms/mobile/auth/presentation/pages/auth.dart';
      ...
      child: ElevatedButton(
        ...
        onPressed: () async {
          void continueAuth() {
            switch (widget.loginMode) {
              case LoginMode.google:
                sl<AuthBloc>().add(AuthenticateWithGoogleEvent(context));
                break;
              case LoginMode.apple:
                sl<AuthBloc>().add(AuthenticateWithAppleEvent(context));
                break;
              case LoginMode.phone:
          ...
          if (sl<AuthBloc>().state is AuthenticatingWithGoogleState ||
              sl<AuthBloc>().state is AuthenticatingWithAppleState) {
            return;
          }
